...
# ══════════════════════════════════════════════════════════════
# Orquestador principal del motor DTE - Corrección de Firma
# ══════════════════════════════════════════════════════════════
...
TIPOS_SIGLAS = {33: "F", 34: "FE", 39: "B", 41: "BE", 52: "G", 56: "ND", 61: "NC"}
...
class DTEService:
    ...
    async def emitir(self, emisor_id: int, datos: dict, auto_enviar: bool = True) -> dict:
        ...
        # 1. Construir XML base
        input_dte = self._construir_input(datos, folio, emisor)
        builder = XMLBuilder(input_dte)
        xml_sin_firma = builder.construir()

        # 2. Proceso de Firma (Punto Crítico)
        cert = emisor.certificado_activo
        # Si el certificado está vacío en la BD, aquí se detendrá con un error claro
        if not cert or not cert.certificado_p12:
            logger.error(f"Certificado no encontrado para emisor {emisor.rut}")
            raise ValueError(f"El emisor {emisor.rut} no tiene un certificado digital activo cargado.")
        ...
        try:
            ...
            logger.error(f"Falla en el proceso de firma digital: {str(e)}")
            raise RuntimeError(f"Error al firmar el XML: {str(e)}")
        ...
        nuevo_dte = DTE(
            ...
            xml_firmado     = xml_firmado_str, # Se garantiza que tiene valor aquí
        ...
        for i, item_data in enumerate(input_dte.items, 1):
            self.db.add(ItemDTE(
                ...
                monto_item=item_data.monto_item, codigo=item_data.codigo
            ))

        await self.db.commit()
        return {"id": nuevo_dte.id, "folio": folio, "status": "success"}

    def _construir_input(self, datos: dict, folio: int, emisor: Emisor) -> InputDTE:
        r_data = datos.get("receptor", {})
        return InputDTE(
            tipo_dte=datos["tipo_dte"], folio=folio,
            ...
            receptor=ReceptorDTE(
                rut=r_data.get("rut"), razon_social=r_data.get("razon_social"),
                giro=r_data.get("giro", "Particular"), direccion=r_data.get("direccion", "Ciudad"),
                comuna=r_data.get("comuna", "Santiago"), ciudad=r_data.get("ciudad", "Santiago")
            ),
            items=[ItemDTEInput(
                nombre=i["nombre"], cantidad=float(i.get("cantidad", 1)),
                precio_unitario=float(i["precio_unitario"]), codigo=i.get("codigo", ""),
