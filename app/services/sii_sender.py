...
from app.services.http_client import get_sii_client  # proxy Chile para SII
    ...
    async def enviar_sobre(self, sobre_xml: str, rut_emisor: str,
        ...
        try:
            async with get_sii_client(timeout=120.0) as client:
                response = await client.post(url_envio, headers=headers, files=files, follow_redirects=True)
